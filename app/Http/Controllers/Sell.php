...
class Sell extends Controller
{
    ...
    public function db_user($_user_id, $_start_time = null, $_end_time = null)
    {
        ...
        if(isset($_start_time) && isset($_end_time))
        {
            $db_result = app('db')->table('sell_order')
                ...
                ->where(
                    [
                        ['sell_order.user_id', $_user_id],
                        ['sell_order.order_timestamp', ">", $_start_time],
                        ['sell_order.order_timestamp', "<", $_end_time],
                    ])
                // ->join('user',      "sell_order.user_id"      ,'=',   "user.id")
                ->get();
        }
        else
        {
            $db_result = app('db')->table('sell_order')
                ->select('id', 'user_id', 'order_quantity as quantity', 'order_price as price', 'order_timestamp as timestamp')
                ->where('sell_order.user_id', $_user_id)
                // ->join('user',      "sell_order.user_id"      ,'=',   "user.id")
                ->get();
        }
        return $db_result;
    ...
    public function set(Request $_req)
    {
        ...
        $response["status"]  = "ok";
        $response["message"] = "sell_order with id: ".$id." add to db";
        return response()->json($response);
    }
    /**
     * [db_set description]
    ...
    public function update(Request $_req)
    {
        ...
        $response["status"]  = "ok";
        $response["message"] = "sell_order updated with new id: ".$id;
        return response()->json($response);
    ...
    public function db_update($_id, $_new_quantity, $_new_price)
    {
        ...
        $user_id = app('db')->table('sell_order')
            ->select('user_id')
            ->where('id', $_id)
            ->get();
        
        app('db')->table('sell_order')
            ->where('id', $_id)
            ->delete();
